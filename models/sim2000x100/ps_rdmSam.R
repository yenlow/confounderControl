# To simulated data 2000x100, do random sampling repeatedly
# 
# 29-Apr-14 Yen Low
##############################################################################
...
matchedID=cbind(ds[ds[,exposed]==1,id],sample(ds[ds[,exposed]==0,id],nminor,replace=FALSE))
rdm1xResults=extractResultsRdm(matchedID,ds,fmod=NULL,id=id,exposed=exposed,outcome=outcome,verbose=FALSE)
...
start=proc.time()[1:3]
for(j in 1:Ndraw){
  matchedID=cbind(ds[ds[,exposed]==1,id],sample(ds[ds[,exposed]==0,id],nminor,replace=TRUE))
  rdmBagResults=extractResultsRdm(matchedID,ds,fmod=NULL,id=id,exposed=exposed,outcome=outcome,verbose=FALSE)
...
#jackknife by shuffling PS scores and then resample
  matchedID=cbind(ds[ds[,exposed]==1,id],sample(ds[ds[,exposed]==0,id],nminor))
  rdmJKResults=extractResultsRdm(matchedID,ds,fmod=NULL,id=id,exposed=exposed,outcome=outcome,verbose=FALSE)
...
if(i %in% seq(50,900,by=50)) save(resultsArray,time,Noutcomesmat,Nexposed,trueORvec,trueSMDvec,trueCoeffvec,file="sim2000x100_1000MC_rdm.RData")
...
save(resultsArray,time,Noutcomesmat,Nexposed,
     trueORvec,trueSMDvec,trueCoeffvec,bestlambda,file="sim2000x100_1000MC_rdm.RData")
