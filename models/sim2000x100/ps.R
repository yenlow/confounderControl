...
#source("../../rfFunctions.R")    #if using padjfromRF
...
hdpsobj=hdps(datainstring,dimdata,outDir=tempDir,Nmostfreq=50,k=50,stratifyDim=FALSE,
...
xmat=as.matrix(ds10[,paste("x",1:100,sep="")])
...
xmat=Matrix(as.matrix(ds10[,c(exposed,paste("x",1:100,sep=""))]),sparse=T)
penalty.factor=c(0,rep(1,ncol(xmat)-1)) #set 0 to force variable into model
#tune lambda for glmnet using 5-fold CV
...
pmatArray[,,i]=t(pmat[paste("x",1:100,sep=""),])
smdmatArray[,,i]=t(smdmat[paste("x",1:100,sep=""),])
...
##########save periodidically
  if(i %in% seq(50,900,by=50)){
    save(resultsArray,time,wantedVarList,rfImpt,lassoPSBeta,lassoMVvar,bestlambda,var_corOutcomeList,matchedID,
         Noutcomesmat,Nexposed,trueORvec,trueSMDvec,trueCoeffvec,pmatArray,smdmatArray,file="sim2000x100_1000MC.RData")
  }
...
dimnames(pmatArray)[1:2]=list(colnames(pmat),paste("x",1:100,sep=""))
dimnames(smdmatArray)[1:2]=list(colnames(smdmat),paste("x",1:100,sep=""))
save(resultsArray,time,wantedVarList,rfImpt,lassoPSBeta,lassoMVvar,bestlambda,var_corOutcomeList,matchedID,
     Noutcomesmat,Nexposed,trueORvec,trueSMDvec,trueCoeffvec,pmatArray,smdmatArray,file="sim2000x100_1000MC.RData")
