...
# force age, sex into model where possible => forcedVar (expertPS, hdPS, lassoPS, lassoMV)
# use previously stitched hdpsobj for full looping through 20 outcomes 
# (remove first variable, ie sex if selected because it is already forced in later)
# 
# 10-Sep-14 Yen Low
##############################################################################
#
...
forcedVar=c("min_pad_age_scaled","gender.male")
...
penalty.factorMV[colnames(smat) %in% c("cilostazol",forcedVar)]=0
...
load(file="hdpsobj_stitched.RData",verbose=T)
...
#special handling for gender.male = D01V001Once (remove from hdps if selected since it is forced into model)
dataPS=cbind(ds[,c(id,exposed,var_corOutcome,forcedVar)],hdpsobj[[i]]$hdpsdata[,setdiff(wantedvar,"D01V001Once")])
